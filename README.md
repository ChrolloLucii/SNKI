...
7. [С чего начать backend-разработчику](#7-с-чего-начать-backend-разработчику)
...
## 7. С чего начать backend-разработчику

Если вы backend-разработчик и выбираете первую задачу:

1. Возьмите issue из колонки **Ready**.
2. Фильтруйте по label `area:api`.
3. Для первой задачи выбирайте:
   - `type:bug` + `area:api`, или
   - `type:tech` + `area:api`.
4. Предпочтительно начать с `priority:should` (задачи `priority:must` могут требовать больше контекста).

### Минимальный стартовый план

1. Прочитайте [docs/API.md](docs/API.md) и [docs/ARCHITECTURE.md](docs/ARCHITECTURE.md).
2. Создайте ветку: `git checkout -b fix/<ticket>-short-desc` или `tech/<ticket>-short-desc`.
3. Реализуйте изменение в backend-части (`apps/api`), держите PR небольшим.
4. Откройте PR по [шаблону](.github/pull_request_template.md) и запросите ревью у команды.
