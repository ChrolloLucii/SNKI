...
foreach ($uid in $users) {
    ...
    try {
        $r = Invoke-WebRequest -Uri "$ApiUrl/slots/$slotId/join" -Method POST -Headers @{ "X-Demo-Token" = $uid } -ContentType "application/json" -Body "{}" -UseBasicParsing -ErrorAction Stop
