...
$joinResp = curl.exe -s -X POST -H "X-Demo-Token: $user" -H "Content-Type: application/json" -d "{\`"user_id\`":\`"$user\`"}" http://localhost:8080/slots/$slot/join
...
$pay1Resp = curl.exe -s -X POST -H "X-Idempotency-Key: $idempotencyKey1" -H "X-Demo-Token: $user" -H "Content-Type: application/json" -d $body http://localhost:8080/slots/$slot/pay
Write-Host "Response: $pay1Resp" -ForegroundColor Green

Write-Host "`n3. Simulating Network Retry (Same Key)..." -ForegroundColor Yellow
$pay2Resp = curl.exe -s -X POST -H "X-Idempotency-Key: $idempotencyKey1" -H "X-Demo-Token: $user" -H "Content-Type: application/json" -d $body http://localhost:8080/slots/$slot/pay
...
$pay3Resp = curl.exe -s -X POST -H "X-Idempotency-Key: $idempotencyKey2" -H "X-Demo-Token: $user" -H "Content-Type: application/json" -d $body http://localhost:8080/slots/$slot/pay
